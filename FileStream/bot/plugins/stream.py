...
async def channel_receive_handler(bot: Client, message: Message):
    ...
    try:
        ...
        try:
            ...
            logging.error(f"Permission Error: Bot needs 'Edit Messages' permission in channel {message.chat.title} ({message.chat.id}) to update buttons.")
        ...
        # Prevent crash if ULOG_CHANNEL is inaccessible (ChannelPrivate error)
        try:
            await bot.send_message(chat_id=Telegram.ULOG_CHANNEL, text=f"**#EʀʀᴏʀTʀᴀᴄᴋᴇʙᴀᴄᴋ:** `{e}`",
                                   disable_web_page_preview=True)
        except Exception:
            logging.error(f"Failed to send error log to ULOG_CHANNEL. Original error: {e}")